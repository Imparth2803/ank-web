...
Single responsibility: transformed DataFrame + Summary + metadata -> .xlsx file.
All calculated cells use Excel formulas (SUM, division) so the sheet stays dynamic.
No ANK logic here — only presentation.
"""
...
class ExportMeta:
    company_name:    str
    client_name:     str
    from_date:       str    # display string e.g. "01-04-2024"
    to_date:         str
    interest_rate:   float
...
def export(
    ...
    row += 1
    row = _write_summary(ws, summary, meta, data_start, data_end, row)
...
def _write_title(ws, meta: ExportMeta, row: int) -> int:
    cell = ws.cell(row=row, column=1, value=f"Company Name: {meta.company_name}")
...
def _write_meta_rows(ws, meta: ExportMeta, row: int) -> int:
    # Row 1: From Date | To Date | Interest | DR DAYS
    pairs_r1 = [
    ...
    # Row 2: Client Name | CR DAYS | Manual Interest (if set)
...
def _write_column_headers(ws, row: int) -> int:
    headers = [
        "DATE", "BILL NO.", "DEBIT AMT.", "DAYS", "ANK AMT.",
...
def _write_summary(
    ...
    ws.cell(row=row, column=2,
            value=f"=C{tot_row}-F{tot_row}").number_format = "#,##0.00"
    _bold_cell(ws, row, 4, "Total ANK:")
    ws.cell(row=row, column=5,
            value=f"=E{tot_row}-H{tot_row}").number_format = "#,##0.00"
    row += 2

    # Interest — manual (blue) or formula
    _bold_cell(ws, row, 1, "Interest")
    if meta.manual_interest:
        c = ws.cell(row=row, column=3, value=meta.manual_interest)
        c.font = Font(name=S.body_font_name, color="0000FF")  # blue = user input
    else:
        c = ws.cell(row=row, column=3,
                    value=f"=(C{tot_row}*{rate})/30")
    ...
    c = ws.cell(row=row, column=3,
                value=f"=(E{tot_row}-H{tot_row})*{rate}")
    c.number_format = "#,##0.00"
    ...
    if meta.debit_days or meta.credit_days:
        note = f"DR offset: -{meta.debit_days}d  |  CR offset: -{meta.credit_days}d"
